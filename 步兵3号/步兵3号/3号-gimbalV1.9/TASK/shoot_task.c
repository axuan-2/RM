...
float TriggerSpeed[5] 	= {2.92f,			0.08f,			0.0f,		8000,			500};  //拨弹轮速度环
//float TriggerAngle[5] 	= {0.2f,			0.0000f,			0.00f,		4800,			0};  //拨弹轮角度环
float TriggerAngle[5] 	= {0.2f,			0.0000f,			0.00f,		3200,			0};  //拨弹轮角度环
...
static void trigger_angle_set(void)
{
	int angle_max;
	int angle_min;
	angle_max=((SHOOT_NUM*40*36)/360*8191);
	angle_min=((SHOOT_NUM*39*36)/360*8191);
	...
	if(One_Shoot_flag==1)
	{
		if(shoot_times==6)
		{
			rc_shoot.trigger.target_angle+=angle_max+5;
		}
		else
		{
			if(shoot_times%2==0)rc_shoot.trigger.target_angle+=angle_max;else rc_shoot.trigger.target_angle+=angle_min;
		}
	}
	else 
	{
		rc_shoot.trigger.target_angle+=((SHOOT_NUM*2*36*36)/360*8191);
	}
