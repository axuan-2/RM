...
//			gimbal_set_mode=GIMBAL_ZERO_FORCE;
	...
	if(switch_is_down(rc_ctrl.rc.s[1])) // 下  发子弹
	{
		...
		{
			rc_shoot.left_fric.target_speed = -SHOOT_FRIC_SPEED_MAX;
		    rc_shoot.right_fric.target_speed = SHOOT_FRIC_SPEED_MAX;
		}
...
void key_control_data(void)
{
	...
	/* 能量机关击打状态判断*/
	if(fricspeed==FRIC_MAX && vision_mode==VISION_ON)
	{
		gimbal_set_mode=GIMBAL_RELATIVE_ANGLE;
	}
	else if(fricspeed==FRIC_MAX && vision_mode==VISION_OFF && gimbal_set_mode!=GIMBAL_TOP_ANGLE)
	{
		gimbal_set_mode=GIMBAL_ABSOLUTE_ANGLE;
		gimbal_y.target_angle=gimbal_y.IMU_actual_angle;
		rc_sent.yaw.target_angle=gimbal_y.IMU_actual_angle;
	}
